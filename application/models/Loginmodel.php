 ...
 function login($username,$password)
  ...
  function getuserinfogoogle($email,$firstname,$lastname){
   ...
   function getuserfb($firstname,$email){
	...
	{
    if($gateway=='Paytm'){
			$sql="SELECT A.id, A.order_id, E.category_name, B.id AS event_id, B.event_name, B.event_banner, B.description, B.event_venue, B.event_address, C.show_date, C.show_time, D.plan_name, A.number_of_seats, A.total_amount, A.created_at, F.*, um.id, um.user_name, um.mobile_no, um.email_id, ud.name FROM booking_history A, events B, booking_plan_timing C, booking_plan D, category_master E, booking_status_paytm F, user_master um, user_details ud WHERE A.order_id = '$order_id' AND F.order_id = '$order_id' AND A.event_id = B.id AND A.plan_time_id = C.id AND A.plan_id = D.id AND B.category_id = E.id AND A.user_id = um.id AND A.user_id = ud.user_id ";
		
      	 	//$sql="SELECT A.id,A.order_id,E.category_name,B.id AS event_id,B.event_name,B.event_banner,B.description,B.event_venue,B.event_address,C.show_date,C.show_time,D.plan_name,A.number_of_seats, A.total_amount,A.created_at, F.* FROM booking_history A,events B,booking_plan_timing C,booking_plan D,category_master E, booking_status_paytm F  WHERE A.order_id  = '$order_id' AND F.order_id  = '$order_id' AND A.event_id = B.id AND A.plan_time_id = C.id AND A.plan_id = D.id AND B.category_id = E.id";
    }else{
      	 	$sql="SELECT A.id,A.order_id,E.category_name,B.id AS event_id,B.event_name,B.event_banner,B.description,B.event_venue,B.event_address,C.show_date,C.show_time,D.plan_name,A.number_of_seats, A.total_amount,A.created_at, F.*, um.id, um.user_name, um.mobile_no, um.email_id FROM booking_history A,events B,booking_plan_timing C,booking_plan D,category_master E, booking_status F,user_master um WHERE A.order_id = '$order_id' AND F.order_id = '$order_id' AND A.event_id = B.id AND A.plan_time_id = C.id AND A.plan_id = D.id AND B.category_id = E.id AND A.user_id = um.id";
    }
