...
function view_booking_history_details()
{
  ...
  $sql="SELECT h.*,p.plan_name,e.event_name,bt.show_date,bt.show_time,bt.seat_available FROM booking_history AS h,booking_plan AS p,events AS e,booking_plan_timing AS bt WHERE h.plan_id=p.id AND h.event_id=p.event_id AND h.event_id=e.id AND h.plan_time_id=bt.id ORDER BY h.id DESC";
