...
{
  $sql="SELECT bp.*,p.plan_name,e.event_name FROM booking_process AS bp,booking_plan AS p,events AS e WHERE bp.plan_id=p.id AND bp.event_id=p.event_id AND bp.event_id=e.id ORDER BY bp.id DESC";
  $resu=$this->db->query($sql);
  $res=$resu->result();
  return $res;	
}
